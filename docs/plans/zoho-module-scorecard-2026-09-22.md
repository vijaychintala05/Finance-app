...
| Error experience | Partial | 5 | A shared operation-receipt contract and accessible banner now distinguish confirmed success, deterministic rejection, uncertain mutation outcome, and committed-but-stale refresh state while exposing server recovery guidance and request IDs. Team Access, recurring transactions, and receivable/payable settlement use it. However, 62 blocking `alert`/`confirm` calls remain across 24 component files. | Migrate invoices, bills, expenses, purchase orders, customers, and remaining settings flows; add field-error mapping, focus management, and copyable support evidence. |
...
5. Standardize financial error/uncertain-outcome UX and request-ID support. In progress: the API boundary now preserves the full structured error contract and all requests carry trace IDs; one shared receipt model serves Team Access, recurring transactions, and settlements. Remaining: migrate 62 blocking dialogs in 24 component files and normalize controller-local server envelopes.
