...
function getOrgId(req: Request): string {
  return (req as any).auth?.organizationId || (req as any).organizationId || (req.query?.orgId as string) || (req.body?.organizationId as string) || 'org-default';
}
...
export class BankingController {
  ...
  public static async createAccount(req: Request, res: Response) {
    try {
      const orgId = getOrgId(req);
      const account = await BankReconciliationService.createBankAccount(orgId, req.body);
  ...
  public static async importStatement(req: Request, res: Response) {
    try {
      ...
      if (transactions && Array.isArray(transactions)) {
        ...
        const importId = `stmt-${Date.now()}`;
        for (const tx of transactions) {
          const fp = `${tx.transactionDate || tx.date}_${tx.amount}_${tx.reference || tx.narration || ''}_${Math.random()}`;
          await db.query(
            ...
            [
              `btx-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
              orgId,
              bankAccountId,
              importId,
  ...
  public static async matchTransaction(req: Request, res: Response) {
    try {
      ...
      const userEmail = (req as any).user?.email || (req as any).auth?.userId || 'User';
  ...
  public static async unmatchTransaction(req: Request, res: Response) {
    try {
      ...
      const ok = await BankReconciliationService.unmatchTransaction(orgId, matchId, (req as any).user?.email || 'User');
      res.json({ success: ok });
    } catch (e: any) {
      res.status(500).json({ success: false, error: e.message });
    }
  ...
  public static async completeSession(req: Request, res: Response) {
    try {
      const orgId = (req as any).organizationId || (req.body.organizationId as string) || 'org-default';
