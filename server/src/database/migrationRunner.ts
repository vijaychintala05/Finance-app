...
export class MigrationRunner {
  public static async runMigrations(queryClient?: { query: (text: string, params?: any[]) => Promise<DbQueryResult> }): Promise<void> {
    ...
    const tables = [
      ...
      `CREATE INDEX IF NOT EXISTS idx_journal_entries_org_status_date ON journal_entries (organization_id, status, date)`,
      ...
      `CREATE INDEX IF NOT EXISTS idx_invoices_org_issue_status ON invoices (organization_id, issue_date, status)`,
      `CREATE INDEX IF NOT EXISTS idx_bills_org_bill_status ON bills (organization_id, bill_date, status)`,
