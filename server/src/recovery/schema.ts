...
function tenantTable(name: string, columns: readonly string[]): RecoveryTableSchema {
  return {
    ...
    columns,
    tenantColumn: 'organization_id',
    selectSql: `SELECT ${columns.join(', ')} FROM ${name} WHERE organization_id = $1 ORDER BY id`,
...
export const POINT1_RECOVERY_SCHEMA: readonly RecoveryTableSchema[] = [
  ...
  tenantTable('vendors', ['id', 'organization_id', 'name', 'company_name', 'email', 'phone', 'billing_address', 'payables_balance', 'created_at']),
