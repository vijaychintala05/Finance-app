...
export class GlobalSearchService {
  public static async search(
    ...
    queryStr: string,
    permissionsOrLimit?: string[] | number,
    limitArg?: number
    ...
    if (Array.isArray(permissionsOrLimit)) {
      permissions = permissionsOrLimit;
      if (typeof limitArg === 'number') maxLimit = limitArg;
    } else if (typeof permissionsOrLimit === 'number') {
      maxLimit = permissionsOrLimit;
    }
      ...
      hasSalesPerm
        ? db.query(
            `SELECT id, display_name, legal_name, email, gstin, phone FROM customers
             WHERE organization_id = $1 AND (LOWER(display_name) LIKE $2 OR LOWER(legal_name) LIKE $2 OR LOWER(email) LIKE $2 OR LOWER(COALESCE(gstin, '')) LIKE $2 OR LOWER(COALESCE(phone, '')) LIKE $2) LIMIT 10`,
            [organizationId, q]
          )
        : Promise.resolve({ rows: [], rowCount: 0 }),
      hasPurchasesPerm
        ? db.query(
            `SELECT id, name, company_name, email, phone, tax_id FROM vendors
             WHERE organization_id = $1 AND (LOWER(name) LIKE $2 OR LOWER(COALESCE(company_name, '')) LIKE $2 OR LOWER(COALESCE(email, '')) LIKE $2 OR LOWER(COALESCE(phone, '')) LIKE $2 OR LOWER(COALESCE(tax_id, '')) LIKE $2) LIMIT 10`,
            [organizationId, q]
          )
        : Promise.resolve({ rows: [], rowCount: 0 }),
      hasPurchasesPerm
      ...
      hasSalesPerm
        ? db.query(
            `SELECT id, payment_number, client_name, amount, payment_mode, reference, payment_date FROM payments_received
             WHERE organization_id = $1 AND (LOWER(payment_number) LIKE $2 OR LOWER(client_name) LIKE $2 OR LOWER(COALESCE(reference, '')) LIKE $2 OR amount = $3) LIMIT 10`,
            [organizationId, q, numQ]
          )
        : Promise.resolve({ rows: [], rowCount: 0 }),
      hasPurchasesPerm
        ? db.query(
            `SELECT id, payment_number, vendor_name, amount, payment_mode, reference, payment_date FROM payments_made
             WHERE organization_id = $1 AND (LOWER(payment_number) LIKE $2 OR LOWER(vendor_name) LIKE $2 OR LOWER(COALESCE(reference, '')) LIKE $2 OR amount = $3) LIMIT 10`,
            [organizationId, q, numQ]
          )
        : Promise.resolve({ rows: [], rowCount: 0 }),
      hasBankPerm
        ? db.query(
            `SELECT id, narration, reference, amount, direction, reconciliation_status, transaction_date FROM bank_statement_transactions
             WHERE organization_id = $1 AND (LOWER(narration) LIKE $2 OR LOWER(COALESCE(reference, '')) LIKE $2 OR amount = $3) LIMIT 10`,
            [organizationId, q, numQ]
          )
        : Promise.resolve({ rows: [], rowCount: 0 }),
      hasAccountingPerm
    ...
    for (const r of payMadeRes.rows) {
      results.push({
        ...
        url: `/purchases/payments?id=${r.id}`,
      });
    }
    ...
    for (const r of bankTxRes.rows) {
      results.push({
        ...
        url: `/banking/reconciliation?tx=${r.id}`,
      });
    }
    ...
    for (const r of cnRes.rows) {
      results.push({
        ...
        url: `/sales/credit_notes?id=${r.id}`,
      });
    }
