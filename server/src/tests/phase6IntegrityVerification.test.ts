...
describe('Phase 6: General Ledger Integrity & Hardened Verification Test Suite', () => {
  ...
  beforeEach(async () => {
    ...
    await db.query(`
      ...
      VALUES 
        ...
        ('acc-1200', '${ORG_ID}', '1200', 'Input Tax Receivable', 'Asset', 'Other Current Asset', 0.00),
        ('acc-2000', '${ORG_ID}', '2000', 'Accounts Payable', 'Asset', 'Accounts Payable', 0.00),
        ('acc-2200', '${ORG_ID}', '2200', 'Sales Tax Payable', 'Liability', 'Tax Payable', 0.00),
        ('acc-4000', '${ORG_ID}', '4000', 'Sales Revenue', 'Revenue', 'Sales Revenue', 0.00),
        ('acc-5000', '${ORG_ID}', '5000', 'Operating Expense', 'Expense', 'Operating Expense', 0.00),
        ('acc-2100', '${ORG_ID}', '2100', 'Customer Advances', 'Liability', 'Other Current Liability', 0.00)
  ...
  it('7. Tax integrity reconciles both certified tax control accounts and fails on a one-cent mismatch', async () => {
    ...
    await db.query(
      `INSERT INTO bills (id, organization_id, bill_number, vendor_name, bill_date, due_date, subtotal, tax_total, total_amount, amount_paid, balance_due, status)
       VALUES ('bill-tax-integrity', $1, 'BILL-TAX-1', 'Tax Vendor', '2025-04-11', '2025-05-11', 50.00, 9.00, 59.00, 0, 59.00, 'UNPAID')`,
      [ORG_ID]
    );
    await db.query(
      `INSERT INTO journal_entries (id, organization_id, entry_number, date, status, description)
       VALUES
         ('je-tax-output', $1, 'JE-TAX-OUT', '2025-04-10', 'POSTED', 'Output tax posting'),
         ('je-tax-input', $1, 'JE-TAX-IN', '2025-04-11', 'POSTED', 'Input tax posting')`,
      [ORG_ID]
    );
    await db.query(
      `INSERT INTO journal_lines (id, journal_entry_id, account_id, account_code, account_name, debit, credit)
       VALUES
         ('jl-tax-out-ar', 'je-tax-output', 'acc-1100', '1100', 'Accounts Receivable', 118.00, 0),
         ('jl-tax-out-revenue', 'je-tax-output', 'acc-4000', '4000', 'Sales Revenue', 0, 100.00),
         ('jl-tax-out-control', 'je-tax-output', 'acc-2200', '2200', 'Sales Tax Payable', 0, 18.00),
         ('jl-tax-in-expense', 'je-tax-input', 'acc-5000', '5000', 'Operating Expense', 50.00, 0),
         ('jl-tax-in-control', 'je-tax-input', 'acc-1200', '1200', 'Input Tax Receivable', 9.00, 0),
         ('jl-tax-in-ap', 'je-tax-input', 'acc-2000', '2000', 'Accounts Payable', 0, 59.00)`
    );

    const balanced = await AccountingIntegrityService.verifyGSTIntegrity(ORG_ID);
    expect(balanced.isBalanced).toBe(true);
    expect(balanced.expectedAmount).toBe('27.00');
    expect(balanced.actualAmount).toBe('27.00');

    await db.query(`UPDATE journal_lines SET credit = 17.99 WHERE id = 'jl-tax-out-control'`);
    const mismatched = await AccountingIntegrityService.verifyGSTIntegrity(ORG_ID);
    expect(mismatched.isBalanced).toBe(false);
    expect(mismatched.difference).toBe('0.01');
  });
});
