...
describe('Phase 7B — Reporting Services & Cross-Ledger Reconciliation Verification', () => {
  ...
  it('2. Seed Golden Reporting Scenario and Verify Cross-Reconciliations', async () => {
    ...
    const bankAcc = `acc-1010-${orgId}`;
    const arAcc = `acc-1100-${orgId}`;
    const inputGstAcc = `acc-1200-${orgId}`;
    const fixedAssetAcc = `acc-1500-${orgId}`;
    const apAcc = `acc-2000-${orgId}`;
    const outputGstAcc = `acc-2200-${orgId}`;
    const capitalAcc = `acc-3000-${orgId}`;
    const revenueAcc = `acc-4000-${orgId}`;
    const expAcc = `acc-5000-${orgId}`;
    const rentAcc = `acc-5100-${orgId}`;
    const deprExpAcc = `acc-5200-${orgId}`;
    const accumDeprAcc = `acc-1590-${orgId}`;
  ...
  it('4. Report API Endpoints Direct Smoke Test (200 OK & Proper Payload)', async () => {
    ...
    expect(ep5.body.closingCashBalance).toBeDefined();
    ...
    expect(ep8.body.closingBalance).toBeDefined();
    ...
    expect(ep9.body.closingBalance).toBeDefined();
  });
  ...
  it('5. Multi-Tenant Isolation Enforced Across All Financial Reports', async () => {
    ...
    expect(epTB.body.rows.length).toBe(0); // Org B has no financial records
