...
describe('Phase 7 — Security, Roles, Audit Trail, Backup & Production Hardening', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO users (id, email, password_hash, full_name, status)
       VALUES ($1, $2, 'not-used', 'Security Owner', 'Active'),
              ($3, $4, 'not-used', 'Security Sales', 'Active'),
              ($5, $6, 'not-used', 'Security Viewer', 'Active')
       ON CONFLICT DO NOTHING`,
      [USER_OWNER, 'security-owner@example.test', USER_SALES, 'security-sales@example.test', USER_VIEWER, 'security-viewer@example.test']
    );
    ownerToken = JwtAuth.generateToken({ userId: USER_OWNER, email: 'security-owner@example.test' });
    salesToken = JwtAuth.generateToken({ userId: USER_SALES, email: 'security-sales@example.test' });
    viewerToken = JwtAuth.generateToken({ userId: USER_VIEWER, email: 'security-viewer@example.test' });
  ...
  describe('1. RBAC & Granular Role Permissions Matrix', () => {
    ...
    it('allows Owner authorization but keeps uncertified application backups fail-closed', async () => {
      const res = await request(app)
        .post('/api/v1/security/backup')
      ...
      expect(res.body.feature).toBe('application-backup');
    });
  });
