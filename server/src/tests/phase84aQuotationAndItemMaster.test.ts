...
describe('Phase 8.4A.4 — Quotation & Item Master Registry Schema Integrity Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    const customer = await SalesEngine.createCustomer(orgIdA, {
      displayName: 'Quotation Test Customer',
      email: 'quotation-customer@example.test',
    });
  ...
  it('31. Cross-organization itemId reference rejection', async () => {
    ...
    // Do not disclose that the item exists in another tenant.
    ).rejects.toThrow(`Item ${itemB.id} not found`);
  });
  ...
  it('43. Unused item is archived to preserve master-data lineage', async () => {
    ...
    expect(delRes.archived).toBe(true);
    expect((await ItemMasterService.getItem(orgIdA, unused.id)).isActive).toBe(false);
  });
  ...
  it('44. Quotation revision preserves status when newData.status is omitted', async () => {
    ...
    await expect(QuotationEngine.reviseQuotation(orgIdA, qSent.id, {
      status: 'ACCEPTED',
    })).rejects.toThrow('may only be saved as DRAFT or SENT');
  });
  ...
  it('49. Explicit unverified item IDs reject the entire invoice atomically', async () => {
    const invoiceNumber = `INV-UNVERIFIED-${Date.now()}`;
    await expect(SalesEngine.createAndPostInvoice(orgIdA, {
      invoiceNumber,
      customerId: customerIdA,
    ...
    })).rejects.toThrow('does not belong to this organization or is inactive');

    const dbRes = await db.query(
      `SELECT id FROM invoices WHERE organization_id = $1 AND invoice_number = $2`,
      [orgIdA, invoiceNumber]
    );
  ...
  it('54. Normal unused item with healthy complete schema is still archived', async () => {
    ...
    expect(delRes.archived).toBe(true);
    expect((await ItemMasterService.getItem(orgIdA, unusedItem.id)).isActive).toBe(false);
  });
});
