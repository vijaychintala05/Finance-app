...
describe('Phase 8.4B.2C — Round-Off, GST Snapshot & Discount Allocation Integrity', () => {
  ...
  beforeEach(async () => {
    ...
    const projectRes = await request(app)
      ...
      .set(authHeader)
      .send({ code: `PRJ-${timestamp}`, name: 'Integrity Project', customerId });
    projectId = projectRes.body.id;

    const estimateRes = await request(app)
      .post('/api/v1/quotations')
      .set(authHeader)
      .send({ customerId, items: [{ name: 'Snapshot source', quantity: 1, rate: 100, taxRate: 0 }] });
    estimateId = estimateRes.body.quotation.id;
  });
  ...
  it('1. Direct Invoice creation with +0.40 positive Round-Off posts Cr Round-Off Income 0.40 and balances GL', async () => {
    ...
    for (const line of jeRes.rows) {
      ...
      if (line.account_code === '2200') taxCredit = c;
  ...
  it('7. SalesEngine.createAndPostInvoice return model includes all persisted commercial metadata', async () => {
    ...
    expect(invoice.customerSnapshot).toEqual(expect.objectContaining(customerSnapshot));
    expect(invoice.customerSnapshot.customerId).toBe(customerId);
