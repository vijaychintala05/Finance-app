...
describe('Phase 8: Complete End-to-End Business Lifecycle & Production UX Hardening', () => {
  ...
  it('Step 5: Convert Quotation to Sales Order and then to Posted Invoice', async () => {
    // Convert to SO
    const soRes = await request
      .post(`/api/v1/quotations/${quotationId}/convert-so`)
      .set(authHeader);
  ...
  it('Step 7: Bank Statement Import & Reconciliation Matching', async () => {
    ...
    expect([200, 201]).toContain(bAccRes.status);
    const bankAccountId = bAccRes.body.id;

    // Import statement transaction
    const stmtRes = await request
      .post(`/api/v1/banking/accounts/${bankAccountId}/statements/import`)
      .set(authHeader)
      .send({
        fileName: 'HDFC_Statement_March2026.csv',
        fileHash: `hash-${Date.now()}`,
        transactions: [
          {
            transactionDate: '2026-03-05',
            valueDate: '2026-03-05',
            narration: 'NEFT CR-HORIZON TECH-UTR-HORIZON-001',
            reference: 'UTR-HORIZON-001',
            amount: 50000,
            direction: 'CR',
          },
        ],
    ...
    expect([200, 201]).toContain(stmtRes.status);

    // Query unmatched transactions
    const txsRes = await request
      .get(`/api/v1/banking/accounts/${bankAccountId}/transactions?status=UNMATCHED`)
      .set(authHeader);

    expect(txsRes.status).toBe(200);
    expect(txsRes.body.transactions.length).toBeGreaterThan(0);
    const bankTxId = txsRes.body.transactions[0].id;

    // Execute match against payment received
    const matchRes = await request
      .post('/api/v1/banking/reconciliation/match')
      .set(authHeader)
      .send({
        bankTransactionId: bankTxId,
        matchType: 'CUSTOMER_PAYMENT',
        matchEntityId: 'pay-01', // system matches or creates link
        matchedAmount: 50000,
      });
