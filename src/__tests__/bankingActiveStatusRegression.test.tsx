...
vi.mock('../components/banking/RecordBankTransactionModal', () => ({
  RecordBankTransactionModal: ({ isOpen }: { isOpen: boolean }) => isOpen ? <div data-testid="record-bank-transaction-form" /> : null,
}));
...
describe('BankingView active-status regression', () => {
  ...
  it('opens record transaction from the overview before an account is selected', async () => {
    vi.spyOn(BooksContext, 'useBooks').mockReturnValue({
      accounts: [{ id: 'bank-1', code: '1020', name: 'Operating Bank', type: 'Asset', subType: 'Bank', status: 'Active', balance: 0 }],
      journalEntries: [], expenses: [], paymentsReceived: [],
      settings: { currencySymbol: 'INR' }, refreshAccounts: vi.fn(),
    } as any);
    render(<BankingView />);
    await waitFor(() => expect(screen.getByRole('button', { name: 'Record Transaction' })).toBeTruthy());
    fireEvent.click(screen.getByRole('button', { name: 'Record Transaction' }));
    expect(screen.getByTestId('record-bank-transaction-form')).toBeTruthy();
  });
});
