...
describe('Phase 8.4B.2A & 2B — Production Quotation Details & Conversion Test Suite', () => {
  ...
  it('2. Shows loading indicator while quotation details are being fetched', async () => {
    ...
    render(
      <BooksProvider>
        <EstimateDetailsModal isOpen={true} quotationId="q-100" onClose={vi.fn()} currencySymbol="₹" />
      </BooksProvider>
    );
  ...
  it('26. GST-Inclusive details regression: displays Taxable ₹9,000, GST ₹1,620, Grand Total ₹10,620 and NOT Taxable ₹10,620', async () => {
    ...
    render(
      <BooksProvider>
        <EstimateDetailsModal isOpen={true} quotationId="q-inc-100" onClose={vi.fn()} currencySymbol="₹" />
      </BooksProvider>
    );
