...
describe('Phase 8.3B — Global Search Component, Navigation & Security Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO customers (id, organization_id, display_name, legal_name, email, gstin, phone, currency)
    ...
    await db.query(
      `INSERT INTO vendors (id, organization_id, name, company_name, email, phone, tax_id, currency)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'INR')
       ON CONFLICT (id) DO NOTHING`,
