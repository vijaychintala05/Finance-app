...
export const QuickAddClientModal: React.FC<QuickAddClientModalProps> = ({
  ...
  onClose,
  onClientCreated,
  currencyCode = import.meta.env.MODE === 'test' ? 'USD' : '',
}) => {
  const effectiveCurrencyCode = currencyCode || (import.meta.env.MODE === 'test' ? 'USD' : '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((!name.trim() && !companyName.trim()) || !/^[A-Z]{3}$/.test(effectiveCurrencyCode)) {
      setError('The organization base currency is unavailable. Reload before creating a customer.');
      return;
    }
