...
import { Dashboard as StudioDashboard } from './studio/Dashboard';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ onNavigate }) => {
  ...
  const [dashboardViewMode, setDashboardViewMode] = useState<'operations' | 'finance'>('operations');
  ...
  const Metric = ({
    ...
    <button
      ...
      onClick={onClick}
      disabled={!onClick}
      className={`group flex min-w-0 flex-col justify-between rounded-lg border border-slate-200/90 bg-white p-4 text-left shadow-xs transition-all dark:border-slate-800/90 dark:bg-slate-900 ${
        onClick
          ? 'cursor-pointer hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md dark:hover:border-slate-700'
          : 'cursor-default'
      }`}
    >
      <div className="flex w-full flex-col items-start gap-2 sm:flex-row sm:justify-between">
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0 pt-0.5">
            <h2 className="break-words text-xs font-bold uppercase leading-4 tracking-wider text-slate-500 dark:text-slate-400">{title}</h2>
          </div>
        </div>
        {badge && (
          <span className={`inline-flex max-w-full shrink-0 truncate rounded-full px-2 py-0.5 text-[11px] font-bold ${badgeTone}`}>
            {badge}
          </span>
        )}
      </div>
      ...
      <div className="mt-3.5 space-y-2">
        <div className={`font-financial min-w-0 break-words text-xs font-black leading-5 tracking-tight sm:text-[1.7rem] sm:leading-8 ${tone}`}>{value}</div>
        {progressPercent !== undefined && progressLabel && (
          <div className="space-y-1 pt-1">
            <div className="flex min-w-0 items-center justify-between gap-2 text-[11px] font-semibold text-slate-500 dark:text-slate-400">
        ...
        <div className="flex min-w-0 items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <span className="min-w-0 flex-1 truncate font-medium">{subtitle}</span>
          {onClick && (
  ...
  if (dashboardViewMode === 'operations') {
    return (
      <div className="space-y-4">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 pt-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dashboard Mode:</span>
            <div className="flex rounded-xl bg-slate-100 p-1 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700">
              <button
                type="button"
                onClick={() => setDashboardViewMode('operations')}
                className="px-3 py-1.5 text-xs font-bold rounded-lg bg-white text-emerald-700 shadow-2xs dark:bg-slate-900 dark:text-emerald-400 cursor-pointer"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-bold rounded-lg text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white cursor-pointer"
              >
        ...
        <StudioDashboard
          ...
          onOpenNewTask={() => onNavigate('projects')}
      ...
      <header className="flex flex-col gap-4 border-b border-slate-200/90 bg-white px-1 py-4 dark:border-slate-800/90 dark:bg-slate-950 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-1">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dashboard:</span>
            <div className="flex rounded-xl bg-slate-100 p-1 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700">
              <button
                type="button"
                onClick={() => setDashboardViewMode('operations')}
                className="px-3 py-1 text-xs font-bold rounded-lg text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white cursor-pointer"
              >
                Live Operations Center
              </button>
              <button
                type="button"
                onClick={() => setDashboardViewMode('finance')}
                className="px-3 py-1 text-xs font-bold rounded-lg bg-white text-blue-700 shadow-2xs dark:bg-slate-900 dark:text-blue-400 cursor-pointer"
              >
                Financial Command Center
              </button>
            </div>
