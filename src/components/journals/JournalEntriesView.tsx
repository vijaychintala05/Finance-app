    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="min-h-[40px] bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-xs font-semibold flex items-center space-x-2 shadow-sm transition-all cursor-pointer active:scale-98"
        >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center">
        <div className="relative w-full max-w-md">
          ...
          <input
            ...
            className="w-full min-h-[38px] bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 pl-9 pr-3 py-2 rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredJournals.length === 0 ? (
          <EmptyStateCard
            icon={Calculator}
            title={search ? 'No matching journal entries' : 'No manual journal entries yet'}
            description={
              search
                ? `No entries match "${search}". Try checking the voucher number, reference, or description.`
                : 'Double-entry journal vouchers let you record direct general ledger adjustments with strictly balanced debits and credits.'
            }
            actionLabel={search ? 'Clear Search' : 'New Journal Entry'}
              ...
              >
                ...
                <div className="border border-slate-200/80 dark:border-slate-800 rounded-xl overflow-hidden text-xs">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 uppercase text-[10px]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {jrn.lines.map((l) => (
                        <tr key={l.id}>
                          <td className="p-2.5 pl-3 font-mono text-blue-600 dark:text-blue-400 font-semibold">
                            {l.accountCode}
                          </td>
                          <td className="p-2.5 font-medium text-slate-800 dark:text-slate-200">
                            {l.accountName}
                          </td>
                          <td className="p-2.5 text-right font-mono text-slate-800 dark:text-slate-200 tabular-nums">
                            {l.debit > 0 ? formatCurrency(l.debit, settings.currencySymbol) : '-'}
                          </td>
                          <td className="p-2.5 text-right pr-3 font-mono text-slate-800 dark:text-slate-200 tabular-nums">
