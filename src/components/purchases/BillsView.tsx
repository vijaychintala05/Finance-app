...
export const BillsView: React.FC = () => {
  ...
  const [billNum, setBillNum] = useState(`BILL-2026-00${bills.length + 1}`);
  ...
  const handleCreateBill = (e: React.FormEvent) => {
    ...
    addBill({
      billNumber: billNum || `BILL-2026-00${bills.length + 1}`,
      ...
      amountPaid: 0,
      status: 'Unpaid',
      notes: notes || 'Vendor payable bill',
    });
                ...
                >
                  ...
                  <td className="p-3 text-right pr-4" onClick={(e) => e.stopPropagation()}>
                    {b.status !== 'Paid' && (
                      <button
                        onClick={() => {
                          updateBill(b.id, { amountPaid: b.totalAmount, status: 'Paid' });
                        }}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl border border-slate-200 dark:border-slate-800">
            ...
            <form onSubmit={handleCreateBill} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Bill Reference / Invoice #</label>
                <input
                  type="text"
                  value={billNum}
