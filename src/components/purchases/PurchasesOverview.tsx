...
export const PurchasesOverview: React.FC<PurchasesOverviewProps> = ({ onNavigate }) => {
  const { expenses, bills, purchaseOrders, settings } = useBooks();
  ...
  const outstandingBills = openBills.reduce((sum, bill) => sum + Math.max(0, Number(bill.balanceDue ?? (bill.totalAmount - bill.amountPaid))), 0);
  const activePurchaseOrders = purchaseOrders.filter((order) => !['Closed', 'Cancelled'].includes(order.status));
  const activePurchaseOrderValue = activePurchaseOrders.reduce((sum, order) => sum + Number(order.totalAmount || 0), 0);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs space-y-2">
          ...
          <div className="text-xl font-extrabold text-amber-600 dark:text-amber-400 font-mono">
            {formatCurrency(outstandingBills, settings.currencySymbol)}
          </div>
          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
            {openBills.length} open vendor bills
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs space-y-2">
          ...
          <div className="text-xl font-extrabold text-sky-600 dark:text-sky-400 font-mono">
            {formatCurrency(activePurchaseOrderValue, settings.currencySymbol)}
          </div>
          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
            {activePurchaseOrders.length} active purchase orders
          </div>
        </div>
