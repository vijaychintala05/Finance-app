...
export const PaymentReceivedDetailsModal: React.FC<PaymentReceivedDetailsModalProps> = ({
  ...
  const handleDelete = () => {
    if (confirm(`Are you sure you want to delete payment record ${payment.paymentNumber}?`)) {
      if (deletePaymentReceived) {
        deletePaymentReceived(payment.id);
      }
      onClose();
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 my-auto">
        ...
        <div className="p-5 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="px-3 py-1 bg-teal-100 text-teal-800 border border-teal-200 rounded-full text-xs font-extrabold">
              Received
            </span>
          </div>
