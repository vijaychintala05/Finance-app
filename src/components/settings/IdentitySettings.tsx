...
export const IdentitySettings: React.FC = () => {
  ...
  const handleUpdateProfile = (e: React.FormEvent) => {
    e.preventDefault();
    setMsg({ type: 'error', text: 'Profile edits are unavailable until the verified server update workflow is enabled.' });
  };

  const handlePasswordChange = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newPassword.length < 12) {
      setMsg({ type: 'error', text: 'Password must be at least 12 characters long.' });
      return;
    }
    ...
    const response = await apiClient.post<{ message: string }>('/auth/change-password', { oldPassword, newPassword });
    if (!response.data) {
      setMsg({ type: 'error', text: response.error || 'Password could not be changed.' });
      return;
    }
    setOldPassword('');
    setNewPassword('');
    setConfirmPassword('');
    setMsg({ type: 'success', text: 'Password updated. All sessions were revoked; please sign in again.' });
  ...
  return (
    <div className="space-y-6 text-xs text-slate-700">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-[11px] text-slate-400 font-mono">
                <span>Public User ID: <strong className="text-amber-300 font-bold">{currentUser?.userId || 'Unavailable'}</strong></span>
                <span className="hidden sm:inline">•</span>
                <span>UUID: <strong className="text-slate-300">{userUuid ? `${userUuid.slice(0, 18)}...` : 'Unavailable'}</strong></span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs space-y-4">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={currentUser?.userId || 'Unavailable'}
                disabled
        ...
        {/* Server-managed password card */}
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              <Key className="w-4 h-4 text-indigo-600" />
              <h3 className="text-sm font-bold text-slate-800">Password & Security</h3>
            </div>
            <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-[10px] font-mono font-bold flex items-center gap-1">
              <Fingerprint className="w-3 h-3 text-indigo-600" /> Server managed
            </span>
          </div>
          ...
          <form onSubmit={handlePasswordChange} className="space-y-3">
            ...
            <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-200 text-[11px] text-slate-600 space-y-1">
              <div className="flex items-center gap-1.5 font-bold text-slate-800">
                <History className="w-3.5 h-3.5 text-indigo-600" /> Password security policy
              </div>
              <p className="text-slate-500">
                Passwords are validated and one-way hashed on the server. A successful change revokes every existing token.
              </p>
            </div>
