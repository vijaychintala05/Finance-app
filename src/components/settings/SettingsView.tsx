...
const ServerSettingsUnavailable: React.FC<{ title: string }> = ({ title }) => (
  <div className="rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-900 p-5">
    <div className="flex items-start gap-3">
      ...
      <div>
        <h3 className="text-sm font-bold text-amber-900 dark:text-amber-200">{title} is read-only</h3>
        <p className="text-xs text-amber-800 dark:text-amber-300 mt-1 leading-relaxed">
          This configuration requires a validated, permission-checked server workflow and immutable audit evidence. Browser-only changes and false success messages are disabled.
        </p>
      </div>
    </div>
  </div>
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1400px] mx-auto text-xs min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs">
        ...
        <div className="max-w-md text-right text-[11px] text-slate-500">
          Financial backup, restore, and retention operations are managed by verified server infrastructure, not browser files.
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-9 min-w-0 w-full">
            ...
            {activeTab.startsWith('org-') && (
              <ServerSettingsUnavailable title="Organization configuration" />
            )}

            {/* Users & Roles Subtabs */}
            {activeTab.startsWith('usr-') && (
              activeTab === 'usr-preferences'
                ? <UsersRolesSettings subTab="preferences" />
                : <ServerSettingsUnavailable title="Membership and role management" />
            )}

            {/* Taxes & Compliance Subtabs */}
            {activeTab.startsWith('tax-') && (
              <ServerSettingsUnavailable title="Tax and compliance configuration" />
            )}

            {/* Setup & Configurations Subtabs */}
            {activeTab.startsWith('cfg-') && (
              <ServerSettingsUnavailable title="Accounting setup configuration" />
            )}

            {/* Customization Subtabs */}
            {activeTab.startsWith('cst-') && (
              <ServerSettingsUnavailable title="Document and notification customization" />
            )}
            ...
            {activeTab.startsWith('mod-') && (
              <ServerSettingsUnavailable title="Module configuration" />
            )}

            {/* Online Payments Subtabs */}
            {activeTab.startsWith('pay-') && (
              <ServerSettingsUnavailable title="Online payment integration" />
            )}

            {/* Sales Subtabs */}
            {activeTab.startsWith('sal-') && (
              <ServerSettingsUnavailable title="Sales module configuration" />
            )}

            {/* Purchases Subtabs */}
            {activeTab.startsWith('pur-') && (
              <ServerSettingsUnavailable title="Purchases module configuration" />
            )}
          </div>
        </div>
      )}
